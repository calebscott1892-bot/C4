...
(async()=>{const outDir=process.argv[2]; const cdp=await connect(); await cdp.send('Page.navigate',{url:'http://127.0.0.1:4173/'}); await sleep(1800); await evalValue(cdp,`localStorage.setItem('c4_intro_seen','1'); true`); await cdp.send('Page.reload',{ignoreCache:true}); await sleep(2200); await evalValue(cdp,`(() => { document.querySelectorAll('[aria-label="C4 Studios"]')[1].scrollIntoView({block:'center'}); return true; })()`); await sleep(900); const footer=await evalValue(cdp,`(() => { const footer=document.querySelectorAll('[aria-label="C4 Studios"]')[1]; const r=footer.getBoundingClientRect(); return {x:r.x,y:r.y,width:r.width,height:r.height,cx:r.x+r.width/2,cy:r.y+r.height/2,scrollY:window.scrollY, logoCount: document.querySelectorAll('[aria-label="C4 Studios"]').length}; })()`);
