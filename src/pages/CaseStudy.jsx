...
export default function CaseStudy() {
  ...
  // Only show gallery if at least one screenshot has a real, non-empty URL
  const realScreenshots = study.screenshots?.filter(s => s.url && s.url.trim() !== '') || [];
  const hasScreenshots = realScreenshots.length > 0;
    ...
    <div className="min-h-screen" style={{ backgroundColor: 'var(--c4-bg)' }}>
      ...
      {hasScreenshots && (
        <CaseStudySection title="Screenshots" number={nextNum()} style={{ borderTop: '1px solid var(--c4-border-light)' }}>
          <CaseStudyGallery screenshots={realScreenshots} />
        </CaseStudySection>
      )}
