...
export default function CompactProcess() {
  ...
  const computeStyles = useCallback(() => {
    ...
    // Throttle active index updates to prevent dot flickering
    if (pendingActiveRef.current !== closestIdx) {
      pendingActiveRef.current = closestIdx;
  ...
  const goTo = useCallback((idx) => {
    ...
    if (snapTimerRef.current) clearTimeout(snapTimerRef.current);
    // Cancel pending active throttle
    ...
    setActive(clamped);
    pendingActiveRef.current = clamped;
          ...
          <div className="flex items-center gap-2" role="tablist" aria-label="Process stages">
            {steps.map((s, i) => (
              <button
                ...
                onClick={() => goTo(i)}
                className="group p-1 focus:outline-none focus-visible:ring-1 rounded-full" style={{ '--tw-ring-color': 'var(--c4-ring)' }}
              >
                <div
